...
namespace kf
{
	...
	protected:
		...
		static const int simplex[64][4]; // A lookup table to traverse the simplex around a given point in 4D.
		static double dot(const int g[2], double x, double y) 
		{
			return g[0]*x + g[1]*y; 
		}
		...
		static double dot(const int g[4], double x, double y, double z, double w) 
		{
			return g[0]*x + g[1]*y + g[2]*z + g[3]*w;	
		}
	public:
		double noise(kf::Vector4 coord)
		{
    ...
    int i = floor(coord.x + s);
    int j = floor(coord.y + s);
    int k = floor(coord.z + s);
    int l = floor(coord.w + s);
