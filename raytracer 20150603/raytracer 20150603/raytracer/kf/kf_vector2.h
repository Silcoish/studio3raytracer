...
namespace kf
{
   ...
   public:
      ...
      inline friend Vector2T rotate(const Vector2T &v, float angle)
      {
		...
		inline Vector2T<T> xx() const
		{
			return Vector2T<T>(x, x);
		}

		inline Vector2T<T> xy() const
		{
			return Vector2T<T>(x, y);
		}

		inline Vector2T<T> yx() const
		{
			return Vector2T<T>(y, x);
		}

		inline Vector2T<T> yy() const
		{
			return Vector2T<T>(y, y);
		}

		inline Vector2T<T> perpCW() const
		{
			return Vector2T<T>(-y,x);
	    }
		inline Vector2T<T> perpCCW() const
		{
			return Vector2T<T>(y,-x);
	    }
